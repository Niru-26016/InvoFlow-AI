    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-surface-400 mt-1">Manage invoice confirmations and payments</p>
        </div>
        <Link
          to="/buyer/confirm"
        ...
        >
          Pending Confirmations <ArrowRight size={16} />
        </Link>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={FileText} label="Total Invoices" value={invoices.length} color="primary" />
        <StatCard icon={Clock} label="Pending Confirmation" value={pendingConfirmation.length} color="warning" />
        <StatCard icon={Building} label="Confirmed" value={confirmed.length} color="accent" />
        <StatCard icon={CreditCard} label="Financing Active" value={funded.length} color="primary" />
      </div>
              ...
              <div key={inv.id} className="flex items-center justify-between p-4 bg-surface-800/30 rounded-xl border border-surface-800">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-surface-400">From: {inv.msmeEmail?.split('@')[0] || 'MSME'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
